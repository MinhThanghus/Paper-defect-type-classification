...
for i,a  in enumerate(list_image):
    ...
    if (i + 1) < len(list_image):
        ...
        image_h2, image_w2, _ = img1_read.shape

        img1 = cv2.resize(img1_read, (image_h1, image_w1))
        img2 = cv2.resize(img2_read, (image_h2, image_w2))
        a = random.randint(8,15)
        image_new_1 = img1[int(image_h1/a):image_h1 , int(image_h1/a):image_h1 ]
        image_new_2 = img2[int(image_h1/a):image_h2 , int(image_h1/a):image_h2 ]

        file_name = str(index) + "_xxx" + str(i) + str("_.png")
        addh = cv2.vconcat([image_new_1, image_new_2])
